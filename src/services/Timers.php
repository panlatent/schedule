...
class Timers extends Component
{
    ...
    public function getTimerByScheduleId(int $scheduleId): ?TimerInterface
    {
    ...
    public function saveTimer(TimerInterface $timer, bool $runValidation = true): bool
    {
        ...
        if ($isNewTimer) {
            ...
            $timer->uid = Db::uidById(Table::TIMERS, $timer->id);
        }
...
//        }
    ...
    public function handleChangeTimer(ConfigEvent $event): void
    {
        ...
        $id = Db::idByUid(Table::TIMERS, $uid);
        $isNew = empty($id);
        ...
        if ($isNew) {
            ...
            $config['uid'] = $uid;
            Db::insert(Table::TIMERS, $config);
        } else {
            Db::update(Table::TIMERS, $config, ['id' => $id]);
        }
    ...
    public function handleDeleteTimer(ConfigEvent $event): void
    {
        ...
        Db::delete(Table::TIMERS, ['id' => $timer->id]);
